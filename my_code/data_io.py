...
def read_as_df(basename, type="train"):
    ...
    X = pd.read_csv(basename + '_' + type + '.data', sep=' ', names = np.ravel(feat_name), index_col=False )
    ...
    if isfile(solution_file):
        Y = pd.read_csv(solution_file,header=None) # Same with nominal categories
        XY = X.assign(target=Y.values)          # Add the last column
